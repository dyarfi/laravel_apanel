...
class RolesController extends AuthorizedController {
	...
	public function index()
	{
		...
		// Set return data 
	   	$roles = Input::get('path') === 'restored' ? Role::onlyTrashed()->paginate(4) : Role::paginate(4);
	...
	public function trash($id)
	{
		if ($role = Role::find($id))
		{
			...
			return Redirect::to(route('admin.roles'))->with('success', 'Role Trashed!');
		}
	...
	public function restored($id)
	{
		if ($role = Role::onlyTrashed()->find($id))
		{
			...
			return Redirect::to(route('admin.roles'))->with('success', 'Role Restored!');
		}
	...
	public function delete($id)
	{
		if ($role = Role::onlyTrashed()->find($id))
		{
			...
			return Redirect::to(route('admin.roles'))->with('success', 'Role Deleted!');
		}
	...
	protected function processForm($mode, $id = null)
	{
		...
		if ($messages->isEmpty())
		{
			return Redirect::to(route('admin.roles'))->with('success', 'Role Updated!');;
		}
