...
class SettingsController extends AuthorizedController {
	...
	public function index() {
		...
		// Set return data 
	   	$settings = Input::get('path') === 'restored' ? $this->settings->onlyTrashed()->paginate(4) : $this->settings->paginate(4);
	...
	public function trash($id)
	{
		if ($setting = $this->settings->find($id))
		{
			...
			return Redirect::to(route('admin.settings'))->with('success', 'Setting Trashed!');
		}

		return Redirect::to(route('admin.settings'));
	}
	...
	public function restored($id)
	{
		if ($setting = $this->settings->onlyTrashed()->find($id))
		{
			...
			return Redirect::to(route('admin.settings'))->with('success', 'Setting Restored!');
		}
	...
	{	
		...
		if ($id)
		{
			if ( ! $setting = $this->settings->find($id))
			{
				return Redirect::to(route('admin.settings'))->withErrors('Not found data!');;
			}
		}
		else
		{
	...
	protected function processForm($mode, $id = null)
	{
		...
		if ($messages->isEmpty())
		{
			return Redirect::to(route('admin.settings'))->with('success', 'Setting Updated!');
		}
